...
Guidance for AI coding agents working in this repository. Mirrors [CLAUDE.md](CLAUDE.md) —
...
`experiences`, `stats`, `tech`, `skillGroups`, `systemStatus`). Section components import from
there rather than owning their own content; adding or editing a project/experience entry means
editing this file, not the component that renders it.

**Design system.** [DESIGN_SYSTEM.md](DESIGN_SYSTEM.md) is the source of truth for the Hero and
Projects sections (the "locked Nocturne / 2a direction") — palette, type scale, spacing, the
terminal/mono motif budget, and motion timing are all specified there with exact values. Read it
before touching `components/sections/Hero.tsx`, `components/ProjectCard.tsx`,
`components/sections/Projects.tsx`, or the `nx-*` utility classes in `app/globals.css`. Values not
present in that doc are explicitly marked "undefined" — ask rather than inventing one. The rest of
the site (About, Experience, Skills, Contact, forms, focus rings, breakpoints below 390px) is
outside that locked spec.
